...
void SmartProjectionCumulativeFactorAdder::AddSmartFactor(const FeatureTrack& feature_track,
  ...
  for (auto feature_point_it = feature_track.points.rbegin(); feature_point_it != feature_track.points.rend();
       ++feature_point_it) {
    const auto& feature_point = *feature_point_it;
    if (++num_points_added > params().max_num_points_per_factor) break;
