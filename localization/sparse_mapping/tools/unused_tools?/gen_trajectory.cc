...
DEFINE_int32(num_loops, 3, "The number of turns in the trajectory.");
DEFINE_int32(num_samples, 50, "The number sample points in the trajectory.");
DEFINE_string(trajectory_file, "trajectory.csv", "Save the trajectory to this file.");
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < FLAGS_num_samples; i++) {
    ...
    if (FLAGS_trajectory_type == 1) {
      ...
      double d = 0.1;
      r = mx * (d + r) / (d + 1.0);  // stay away from origin
