...
#define ASSERT_VECTOR2D_NEAR(p1, p2, t) \
  ASSERT_NEAR(p1[0], p2[0], t);         \
  ASSERT_NEAR(p1[1], p2[1], t);
...
void CompareFeatures(sparse_mapping::SparseMap const& map1, sparse_mapping::SparseMap const& map_loopback) {
  EXPECT_EQ(map1.GetNumFrames(), map_loopback.GetNumFrames());
  for (size_t frame = 0; frame < map1.GetNumFrames(); frame++) {
    EXPECT_EQ(map1.GetFrameFilename(frame), map_loopback.GetFrameFilename(frame));
    ...
    EXPECT_NEAR(0, (keys1 - keys2).array().abs().sum(), 1e-6);
...
class SparseMapTest : public ::testing::TestWithParam<Parameters> {
 public:
  ...
  virtual void SetUp() {
    ...
    params.reset(new camera::CameraParameters(Eigen::Vector2i(780, 620), Eigen::Vector2d::Constant(258.5),
...
void mapFiles(std::string const& detector, bool close_loop, std::string* map1, std::string* map2) {
...
TEST_P(SparseMapTest, MapBuilding) {
  ...
  // map_loopback
  ASSERT_VECTOR2D_NEAR(params->GetDistortedSize(), map_loopback.GetCameraParameters().GetDistortedSize(), 1e-3);
  ASSERT_VECTOR2D_NEAR(params->GetDistortedHalfSize(), map_loopback.GetCameraParameters().GetDistortedHalfSize(), 1e-3);
  ASSERT_VECTOR2D_NEAR(params->GetUndistortedSize(), map_loopback.GetCameraParameters().GetUndistortedSize(), 1e-3);
  ASSERT_VECTOR2D_NEAR(params->GetUndistortedHalfSize(), map_loopback.GetCameraParameters().GetUndistortedHalfSize(),
                       1e-3);
  ASSERT_VECTOR2D_NEAR(params->GetCropOffset(), map_loopback.GetCameraParameters().GetCropOffset(), 1e-3);
  ASSERT_VECTOR2D_NEAR(params->GetOpticalOffset(), map_loopback.GetCameraParameters().GetOpticalOffset(), 1e-3);
  ASSERT_VECTOR2D_NEAR(params->GetFocalVector(), map_loopback.GetCameraParameters().GetFocalVector(), 1e-3);
  ASSERT_EQ(params->GetDistortion().size(), 0);
  ASSERT_EQ(map_loopback.GetCameraParameters().GetDistortion().size(), 0);

  // Feature matching
  sparse_mapping::MatchFeatures(sparse_mapping::EssentialFile(mapfile1), sparse_mapping::MatchesFile(mapfile1),
                                &map_loopback);

  // Building tracks
  bool rm_invalid_xyz = true;
  sparse_mapping::BuildTracks(rm_invalid_xyz, sparse_mapping::MatchesFile(mapfile1), &map_loopback);

  // Initialize camera positions
  sparse_mapping::IncrementalBA(sparse_mapping::EssentialFile(mapfile1), &map_loopback);
  if (GetParam().close_loop) sparse_mapping::CloseLoop(&map_loopback);
  ...
  double default_distance =
    (map_loopback.GetFrameGlobalTransform(1).translation() - map_loopback.GetFrameGlobalTransform(0).translation())
      .norm();
  ...
  Eigen::Affine3d closest1 = map_loopback.GetFrameGlobalTransform(1),
  ...
  Eigen::Vector3d close_t1 = closest1.inverse().translation(), close_t2 = closest2.inverse().translation();
  ...
  for (size_t frame = 0; frame < map_loopback.GetNumFrames(); frame++) {
    ...
    std::map<int, int> fidpid1 = map_loopback.GetFrameFidToPidMap(frame),
                       fidpid2 = map_loopback2.GetFrameFidToPidMap(frame);
    ASSERT_EQ(fidpid1.size(), fidpid2.size());
    for (std::map<int, int>::iterator it = fidpid1.begin(); it != fidpid1.end(); it++) {
      EXPECT_EQ(fidpid2[it->first], it->second);
    }
  }
  // Check that landmarks are the same
  ASSERT_EQ(map_loopback.GetNumLandmarks(), map_loopback2.GetNumLandmarks());
  for (size_t pid = 0; pid < map_loopback.GetNumLandmarks(); pid++) {
    EXPECT_VECTOR3D_NEAR(map_loopback.GetLandmarkPosition(pid), map_loopback2.GetLandmarkPosition(pid), 1e-6);
    ...
    for (std::map<int, int>::iterator it = cidpid1.begin(); it != cidpid1.begin(); it++) {
...
TEST_P(SparseMapTest, MapExtractMerge) {
  ...
  sparse_mapping::AppendMapFile(submap1_file, submap2_file, num_image_overlaps_at_endpoints, outlier_factor,
...
INSTANTIATE_TEST_CASE_P(SparseMapTest, SparseMapTest, ::testing::ValuesIn(test_parameters));
