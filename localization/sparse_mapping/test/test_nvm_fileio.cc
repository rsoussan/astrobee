...
TEST(nvm_fileio, read_write_loop) {
  ...
  cid_to_camera_transform[0].linear() = Eigen::Matrix3d(Eigen::AngleAxisd(0.25 * M_PI, Eigen::Vector3d::UnitX()) *
                                                        Eigen::AngleAxisd(-0.25 * M_PI, Eigen::Vector3d::UnitY()));
  cid_to_camera_transform[0].translation() << 100, 101, 102;
  cid_to_camera_transform[1].linear() = Eigen::Matrix3d(Eigen::AngleAxisd(0.7, Eigen::Vector3d::UnitY()) *
                                                        Eigen::AngleAxisd(1.7, Eigen::Vector3d::UnitZ()));
  cid_to_camera_transform[1].translation() << 200, 201, 202;

  // Write it.
  sparse_mapping::WriteNVM(cid_to_keypoint_map, cid_to_filename, pid_to_cid_fid, pid_to_xyz, cid_to_camera_transform,
                           200, "test.nvm");

  // Read it back in
  sparse_mapping::ReadNVM("test.nvm", &cid_to_keypoint_map2, &cid_to_filename2, &pid_to_cid_fid2, &pid_to_xyz2,
  ...
  for (size_t cid = 0; cid < cid_to_keypoint_map.size(); cid++) {
    ASSERT_EQ(cid_to_keypoint_map[cid].cols(), cid_to_keypoint_map2[cid].cols());
    ...
    for (size_t i = 0; i < 12; i++) {
      EXPECT_NEAR(cid_to_camera_transform[cid].data()[i], cid_to_camera_transform2[cid].data()[i], 1e-3);
    }
  }
  for (size_t pid = 0; pid < pid_to_cid_fid.size(); pid++) {
    ASSERT_EQ(pid_to_cid_fid[pid].size(), pid_to_cid_fid2[pid].size());
    for (std::map<int, int>::iterator it = pid_to_cid_fid[pid].begin(); it != pid_to_cid_fid[pid].end(); it++) {
    ...
    for (int i = 0; i < 3; i++) {
      EXPECT_NEAR(pid_to_xyz[pid].data()[i], pid_to_xyz2[pid].data()[i], 1e-3);
    }
  }
}
...
TEST(nvm_fileio, read_write_loop_features) {
  ...
  std::string feat_file = sparse_mapping::ImageToFeatureFile(image_file, descriptor);

  sparse_mapping::WriteFeatures(descriptor, points1, descriptors1, feat_file);
  sparse_mapping::ReadFeatures(feat_file, descriptor, &points2, &descriptors2);
