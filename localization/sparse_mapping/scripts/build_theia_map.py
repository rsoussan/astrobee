...
def process_args(args):
    ...
    parser.add_argument(
        "--output_map", dest="output_map", default="", help="The resulting output map."
    )
    parser.add_argument(
        "--skip_rebuilding",
        dest="skip_rebuilding",
        action="store_true",
...
def gen_theia_calib_file(work_dir, undist_images, undist_intrinsics_file):
    ...
    with open(undist_intrinsics_file, "r") as fh:
        for line in fh:
            ...
            if len(intrinsics) < 5:
                raise Exception(
                    "Expecting 5 intrinsics numbers in: " + undist_intrinsics_file
                )
    ...
    with open(calib_file, "w") as fh:
        ...
        for it in range(num_images):
            ...
            fh.write(
                '"principal_point" : [' + str(opt_ctr_x) + ", " + str(opt_ctr_y) + "]\n"
            )
