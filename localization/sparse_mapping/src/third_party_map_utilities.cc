...
void SparseMap::reorderMap(std::map<int, int> const& old_cid_to_new_cid) {
  ...
  for (size_t pid = 0; pid < pid_to_feature_track_.size(); pid++) {
    auto const& cid_fid = pid_to_feature_track_[pid];  // alias
    ...
    new_pid_to_feature_track[pid] = new_cid_fid;
  }
...
void ReadNVM(std::string const& input_filename, std::vector<Eigen::Matrix2Xd>* cid_to_keypoints,
  ...
  // Read the point
  pid_to_feature_track->resize(number_of_pid);
  pid_to_global_t_point->resize(number_of_pid);
  Eigen::Vector3d xyz;
