...
namespace {
std::vector<ImageMatch> FindAndSortMatches(const std::vector<int>& matching_cids,
  ...
  for (const auto cid : matching_cids) {
    const auto& map_image_descriptors = cid_to_descriptor_map_[cid];
    ...
    if (!check_point_3d_exists) {
      ...
      for (const auto& match : image_match.matches) {
       const bool map_point_3d_exists = cid_fid_to_pid_[cid].count(match.trainIdx) > 0;
...
void GetMatchingObservationsAndLandmarks(const std::vector<ImageMatches>& image_matches,
  ...
  for (const auto& image_match : image_matches) {
    for (const auto& match : image_match.matches) {
      const bool map_point_3d_exists = cid_fid_to_pid_[cid].count(match.trainIdx) > 0;
      if (!map_point_3d_exists) continue;
      const int landmark_id = cid_fid_to_pid_.at(cid).at(match.trainIdx);
...
EstimatePoseResults EstimatePose(const cv::Mat& descriptors, const Eigen::Matrix2Xd& keypoints,
                                 const EstimatePoseParams& params) {
  const auto matching_cids = image_database().Query(descriptors, params.max_image_matches);
  if (matching_cids.empty()) {
    LOG(FATAL) << "No matching cids found.";
  }

  const auto image_matches =
    FindAndSortMatches(matching_cids, params.max_num_total_feature_matches, params.check_point_3d_exists);
  std::vector<Eigen::Vector2d> observations;
  std::vector<Eigen::Vector3d> landmarks;
  GetMatchingObservationsAndLandmarks(image_matches, observations, landmarks);
...
EstimatePoseResults EstimatePose(
  ...
  cv::Mat descriptors;
  Eigen::Matrix2Xd keypoints;
  DetectFeatures(image, params().histogram_equalization, descriptors, keypoints);
