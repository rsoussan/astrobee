...
void EstimateNormals(const typename pcl::PointCloud<PointType>::Ptr cloud, const double search_radius,
                     ...
                     const bool use_integral_image_for_organized_cloud = true);
...
void EstimateNormals(const typename pcl::PointCloud<PointType>::Ptr cloud, const double search_radius,
                     ...
                     const bool use_integral_image_for_organized_cloud) {
  pcl::PointCloud<pcl::Normal>::Ptr cloud_normals(new pcl::PointCloud<pcl::Normal>);
  if (use_integral_image_for_organized_cloud && cloud->isOrganized()) {
    typename pcl::IntegralImageNormalEstimation<PointType, pcl::Normal> ne;
    ne.setNormalEstimationMethod(ne.AVERAGE_3D_GRADIENT);
    // Values taken from pcl example code
