...
void VIOAugmentor::SetBias(Eigen::Vector3f gyro_bias, Eigen::Vector3f accel_bias) {
...
void VIOAugmentor::OpticalFlowUpdate(const ff_msgs::Feature2dArray& of) {
...
void VIOAugmentor::OpticalFlowRegister(const ff_msgs::CameraRegistration& cr) {
...
void VIOAugmentor::SetSpeedGain(const uint8_t gain) { gnc_.cmc_.speed_gain_cmd = gain; }

void VIOAugmentor::SetResetCallback(std::function<void(void)> callback) { reset_callback_ = callback; }

void VIOAugmentor::PrepareStep(const sensor_msgs::Imu& imu, const geometry_msgs::Quaternion& quat) {
  // set IMU values
  // set timestamp
...
int VIOAugmentor::Step(ff_msgs::EkfState* state) {
  gnc_.Step();
