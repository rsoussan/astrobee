...
def run_ekf(astrobee_map,
            ...
            output_file,
            ekf_in_bag=False,
            features_in_bag=False,
            image_topic=None,
            gnc_config='gnc.config'):
...
def covariance_map(ekf, cov_1_index, cov_2_index, cov_3_index):
  return map(lambda (x, y, z): math.sqrt(x + y + z), zip(ekf[cov_1_index], ekf[cov_2_index], ekf[cov_3_index]))

def integrated_velocities(start_x, vs, ts):
  delta_times = [j - i for i, j in zip(ts[:-1], ts[1:])]
  # Make sure times are same length as velocities, ignore last velocity
  delta_times.append(0)
  x_increments = [velocity * delta_t for velocity, delta_t in zip(vs, delta_times)]
  cumulative_x_increments = np.cumsum(x_increments)
  xs = [start_x + cumulative_x_increment for cumulative_x_increment in cumulative_x_increments]
  return xs
...
class EkfLog(object):
  ...
  def evaluate_error(self, time_offset=0.0):
    ...
    for gt_i in range(len(self.gt['t'])):
      ...
      angle_err += float(
        norm(np.array([self.gt['angle1'][gt_i], self.gt['angle2'][gt_i], self.gt['angle3'][gt_i]]) - a))**2
    if err_count == 0:
      return (0.0, 0.0)
  ...
  def rmse_using_single_image_sparse_mapping(self):
    rmse_pos = rmse_timestamped_sequences(self.ekf['x'], self.ekf['y'], self.ekf['z'], self.ekf['t'], self.vl['x'],
                                          self.vl['y'], self.vl['z'], self.vl['t'])
    int_x = integrated_velocities(self.ekf['x'][0], self.ekf['vx'], self.ekf['t'])
    int_y = integrated_velocities(self.ekf['y'][0], self.ekf['vy'], self.ekf['t'])
  ...
  def get_stats(self, job_id):
    ...
    rmse_pos_vs_single_image_sparse_mapping, rmse_angle_vs_single_image_sparse_mapping, rmse_int = self.rmse_using_single_image_sparse_mapping(
    )
    stats.append(rmse_int)
    stat_names.append('integrated_rmse')
  ...
  def plot(self, filename, output_text):
    ...
    with PdfPages(filename) as pdf:
      ...
      int_x = integrated_velocities(ekf['x'][0], ekf['vx'], ekf['t'])
      int_y = integrated_velocities(ekf['y'][0], ekf['vy'], ekf['t'])
      int_z = integrated_velocities(ekf['z'][0], ekf['vz'], ekf['t'])
 
      plt.plot(ekf['t'], int_x, colors[0], linewidth=0.5, label='EKF Pos. (X)')
      plt.plot(ekf['t'], int_y, colors[1], linewidth=0.5, label='EKF Pos. (Y)')
      plt.plot(ekf['t'], int_z, colors[2], linewidth=0.5, label='EKF Pos. (Z)')
      plt.plot(vl['t'],
               vl['x'],
               color=colors[0],
               linestyle='None',
      ...
      plt.plot(vl['t'], vl['z'], color=colors[2], linestyle='None', marker='o', markersize=2)
      plt.xlabel('Time (s)')
      plt.ylabel('Integrated Velocity (m)')
...
def run_ekf_and_save_stats(options):
  (astrobee_map, astrobee_bag, robot_config) = environment.initialize_environment(options.map_file, options.bag_file,
                                                                                  options.robot_name)
  ...
  if not options.cached:
    run_ekf(astrobee_map, astrobee_bag, options.ekf_output_file, options.ekf_in_bag, options.features_in_bag,
            options.image_topic, options.gnc_config)
