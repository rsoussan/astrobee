...
boost::optional<lc::CombinedNavState> firstCombinedNavState(const graph_localizer::GraphLocalizer& graph_localizer,
  ...
  const auto pose = graph_localizer.combined_nav_state_graph_values().Get<gtsam::Pose3>(imu_factor->key1());
  if (!pose) {
    LogError("pimPredict: Failed to get pose.");
    return boost::none;
  }

  const auto velocity = graph_localizer.combined_nav_state_graph_values().Get<gtsam::Velocity3>(imu_factor->key2());
  if (!velocity) {
  ...
  const auto bias =
    graph_localizer.combined_nav_state_graph_values().Get<gtsam::imuBias::ConstantBias>(imu_factor->key5());
  if (!bias) {
